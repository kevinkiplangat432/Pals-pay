...
def get_pending_transactions(current_user):
    ...
    query = Transaction.query.filter_by(status=TransactionStatus.pending)
    query = query.order_by(desc(Transaction.created_at))
...
# Approve transaction
@admin_bp.route('/transactions/<int:tx_id>/approve', methods=['POST'])
@token_required
@role_required('admin')
def approve_transaction(current_user, tx_id):
    transaction = Transaction.query.get_or_404(tx_id)
...
@token_required
@role_required('admin')
def reject_transaction(current_user, tx_id):
    transaction = Transaction.query.get_or_404(tx_id)
    data = request.get_json()
    reason = data.get('reason', 'Rejected by admin')
    
    if transaction.status != TransactionStatus.pending:
        return jsonify({'message': 'Only pending transactions can be rejected'}), 400
