...
class FundingSource(db.Model):
    ...
    # Source details
    source_type = db.Column(db.String(20), nullable=False)  # 'mpesa', 'bank', 'card', 'cash', 'wallet'
    ...
    account_type = db.Column(db.String(30), nullable=True)  # 'savings', 'checking', 'credit_card', etc.
    ...
    verification_method = db.Column(db.String(30), nullable=True)  # 'micro_deposit', 'trial_deposit', 'direct_debit'
    ...
    # Usage tracking
    total_deposits = db.Column(db.Integer, default=0, nullable=False)
    ...
    # Indexes
    __table_args__ = (
        db.Index('idx_funding_sources_user_type', 'user_id', 'source_type'),
        db.Index('idx_funding_sources_country', 'country'),
        db.Index('idx_funding_sources_verified', 'is_verified', 'is_active'),
        db.UniqueConstraint('user_id', 'masked_identifier', name='unique_user_funding_source'),
    )
    ...
    def __repr__(self):
        return f'<FundingSource user_id={self.user_id} type={self.source_type} identifier={self.masked_identifier}>'