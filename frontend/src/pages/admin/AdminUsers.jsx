          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><span className="font-semibold">Name:</span> {selectedUser.first_name} {selectedUser.last_name}</div>
                <div><span className="font-semibold">Email:</span> {selectedUser.email}</div>
                <div><span className="font-semibold">Phone:</span> {selectedUser.phone_number}</div>
                <div><span className="font-semibold">KYC Status:</span> <span className={`px-2 py-1 rounded text-xs ${selectedUser.kyc_status === 'verified' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{selectedUser.kyc_status || 'unverified'}</span></div>
                <div><span className="font-semibold">Status:</span> <span className={`px-2 py-1 rounded text-xs ${selectedUser.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{selectedUser.is_active ? 'Active' : 'Inactive'}</span></div>
